...
function Navbar({ isAuth = false }) {
  ...
  shadow-[0_10px_40px_rgba(0,0,0,0.15)]"
        >
          <div className="mx-auto flex max-w-9/12 items-center justify-between py-3">
            ...
            {/* desktop */}
            <div>
              <ul className="lg:flex items-center gap-6 cursor-pointer hidden text-sm">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                ...
                <li className="rounded-full bg-secondary px-4 py-2 text-white">
                  <Link href="/property">List a Property</Link>
                </li>
              </ul>
            </div>
